...
void test_realloc_block_FF()
{
	...
	*(endVAs[idx]) = idx ;
	...
	//[3.3] reallocate in a different place

//realloc(va,newsize>size) not free
	//va!= null
	//data==data
	//size=newsize
	//status old blck is_free==1

	cprintf("	3.3: reallocate in a different place (NO free block after)\n\n") ;
		is_correct = 1;
		{
			blockIndex = 3*allocCntPerSize+3 ;
			new_size = allocSizes[3] /*12+16 B*/ + allocSizes[4]/2 /*2KB/2*/ - sizeOfMetaData();

			va = realloc_block_FF(startVAs[blockIndex], new_size);

			if(va==NULL){
				cprintf("test_realloc_block_FF #11.1: WRONG REALLOC! The virtual address is null.");
			}
			if (*(startVAs[blockIndex]) != blockIndex || *(midVAs[blockIndex]) != blockIndex)
					{
						cprintf("test_realloc_block_FF #11.2: WRONG REALLOC! content of the block is not correct. Expected %d\n", blockIndex);
					}
			block_size = get_block_size(va) ;
			if (block_size != new_size + sizeOfMetaData())
			{
				cprintf("test_realloc_block_FF #11.3: WRONG REALLOC! block size after realloc is not correct. Expected %d, Actual %d\n",new_size + sizeOfMetaData(), block_size);
			}
...
//shahd
	//[4.2] next block is empty (coalesce)
	cprintf("	4.2: next block is empty (coalesce)\n\n") ;
	{

			free_block(startVAs[0*allocCntPerSize + 3]);
			blockIndex = 0*allocCntPerSize + 2; //4KB
			old_size = allocSizes[0]; //4KB
			new_size = old_size - 1*kilo - sizeOfMetaData();
			va = realloc_block_FF(startVAs[blockIndex], new_size);
			...
			if (*(startVAs[blockIndex]) != blockIndex || *(midVAs[blockIndex]) != blockIndex)
			{
				cprintf("test_realloc_block_FF #15.4: WRONG REALLOC! content of the block is not correct. Expected %d\n", blockIndex);
			}
			//check new free block
			struct BlockMetaData *newBlkMetaData = (struct BlockMetaData *)(va + new_size);
			expected_size = 1*kilo + allocSizes[0] ;
			if (newBlkMetaData->size != expected_size || newBlkMetaData->is_free != 1)
			{
				cprintf("test_realloc_block_FF #15.5: WRONG REALLOC! newly created block is not correct... check it!. Expected %d, Actual %d\n", expected_size, newBlkMetaData->size);
			}
